...
// ../../common/types/facility.ts
function isFacility(obj) {
  return "id" in obj && "name" in obj && "location" in obj && "amenities" in obj && "associatedServiceProviders" in obj;
}

// ../../common/types/provider.ts
function isProvider(obj) {
  return "id" in obj && "firstName" in obj && "lastName" in obj && "specialty" in obj;
}

// src/utils/path2KV.ts
function path2KV(path, env) {
  const resources = {
    facility: { namespace: env.FACILITY, assertType: (obj) => isFacility(obj) },
    provider: { namespace: env.PROVIDER, assertType: (obj) => isProvider(obj) }
  };
  switch (path[0]) {
    case "api":
      return resources[path[1]] ? { kind: "success", value: resources[path[1]] } : { kind: "failure", error: "Could not map KV" };
    case "admin":
      return resources[path[1]] ? { kind: "success", value: resources[path[1]] } : { kind: "failure", error: "Could not map KV" };
    default:
...
// src/thrivapi/crud.ts
async function handleCRUD(request, env) {
